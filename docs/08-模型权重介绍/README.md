...
- [语音模型（唤醒词/PocketSphinx）说明](../03-语音与音频/语音唤醒与指令实现.md)
- [龙芯2K3000手动部署文档](../02-部署与容器/龙芯2K3000手动部署文档.md)
- 环境变量与模型路径：项目根 `.env.example`、`backend/config/settings.py`

## 注意事项

1. **模型文件大小**：真实权重合计约 360MB，部署时需确保存储空间充足
2. **Git LFS（重要）**：本目录下的 `.onnx/.pt` 权重与测试视频走 Git LFS 入库，
   clone 后必须执行 `git lfs pull` 还原真实权重，否则仅有 ~130 字节的
   LFS 指针文件，所有模型加载都会失败（2026-08-28 前后曾被误认为"占位桩"）
3. **不入库的模型**：`pocketsphinx/zh-cn/` 声学模型与 `wakeword/*.onnx`
   训练产物由 `.gitignore` 排除，按各自目录 README 单独下载/训练放置
4. **ONNX 版本兼容**：确保 ONNX Runtime 版本支持模型使用的 opset
5. **量化模型**：INT8 量化模型推理更快但精度略低
6. **默认配置**：人脸检测默认关闭，需手动启用
7. **降级策略**：模型加载失败时自动回退到桩实现，不阻塞主链路
8. **孤儿文件**：`best.pt`、`face_detector.onnx`、`det_500m.onnx`、
   `w600k_mbf.onnx` 当前代码无引用（历史/预留权重），保留在 LFS 中备用
